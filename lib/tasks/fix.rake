...
namespace :fix do
  desc "Imports on v1.0 had timestamps that weren't all accurate. This is an attempt to fix it"
  task imported_timestamps: :environment do
    Purchase.transaction do
      ...
      imported_purchases.find_each.with_index(1) do |purchase, counter|
        ...
        adjust_threshold = (earliest_at - 1.hour)..(earliest_at + 1.hour)
        if earliest_charge.created_at.in?(adjust_threshold)
