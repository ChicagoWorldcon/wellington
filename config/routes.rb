...
Rails.application.routes.draw do
  ...
  get "/login/:email/:key", to: "user_tokens#kansa_login_link", email: %r{[^/]+}, key: %r{[^/]+}
  resources :user_tokens, only: %i[new show create], id: %r{[^/]+} do
  ...
  scope :cart, only: %i[show create edit update] do
    get :new_group_charge, to: "charges#new_group_charge", as: "charge_for_cart"
    resources :cart_items, only: %i[index show edit update] do
      ...
      patch "verify_single_item_availablility", on: :member, to: "cart#verify_single_item_availability",
                                                as: "verify_single"
  ...
  match "charges/cart_payment_confirmation" => "charges#group_charge_confirmation", via: :get,
        :as => "group_charge_confirmation"
  ...
  resources :reservations do
    ...
    resources :charges
    resources :finalists, id: %r{[^/]+}
    resources :nominations, id: %r{[^/]+}
    resources :upgrades
    resources :site_selection_tokens
    resources :token_purchase
  end
