...
#   let(:unpaid_cart_item_ary) create_list(:cart_item, DEFAULT_CART_ITEM_COUNT, :with_unpaid_reservation, cart: whatevs_cart)
#
...
#
#
#   xcontext "when the cart has no charges of its own" do
...
#     end
#
#   xcontext "when the cart has charges of its own, but all are failed or pending" do
#
#     subject { ValidationPotato.new }
#
#     let(:failed_and_pending_charges_ary) { create_list(:charge, DEFAULT_C_ITEM_COUNT_CUBED , amount_cents: ADULT_MEMBERSHIP_PRICE_CENTS, state: Charge::STATE_PENDING, buyable: subject) + create_list(:charge, DEFAULT_C_ITEM_COUNT_CUBED, :failed, amount_cents: ADULT_MEMBERSHIP_PRICE_CENTS, buyable: subject)  }
#
#     subject.charges {:failed_and_pending_charges_ary}
#
#
#     context "when we're pre-validating the test-group" do
#
...
#     end
#
#     context "when the cart contains only items with no holdables and no charges of their own" do
#
...
#     end
#
#     context "when the cart exclusively contains items that have holdables but no charges of their own" do
#
#       subject { ValidationPotato.new(charge_ary: :lots_of_failed_full_membership_charges + :lots_of_pending_full_membership_charges, cart_item_ary: :unpaid_cart_item_ary) }
#
#       context "when we're pre-validating the test-group" do
#         xit 'has a test-cart that is properly set up' do
#           expect(subject).to be
...
#           pending_charges_seen = 0
#           failed_charges_seen = 0
#           cart_item_charges_seen = subject.cart_items.inject(false) {|a, i| a = true if i.charges.present}
#
#           subject.charges.each do |i|
#             successful_charges_seen = true if i.successful?
#             pending_charges_seen_count += 1 if i.pending?
...
#           expect(failed_charges_seen_count).to be > 0
#           expect(pending_charges_seen_count + failed_charges_seen_count).to eql(subject.charges.size)
#           expect(cart_item_charges_seen).to be_falsey
#         end
#       end
#
#       context "when the status is for_now" do
...
#     end
#
#     context "when the cart exclusively contains items that are partially paid through charges of their own" do
#
#       subject { ValidationPotato.new(charge_ary: :lots_of_failed_full_membership_charges + :lots_of_pending_full_membership_charges, cart_item_ary: :partially_paid_cart_item_ary) }
#
#       context "when we're pre-validating the test-group" do
#         xit 'has a test-cart that is properly set up' do
#           expect(subject).to be
...
#           expect(subject.active?).to be_truthy
#
#           cart_item_without_holdable_seen = false
#           cart_item_without_successful_charges_seen = false
#           fully_paid_cart_item_seen = false
#
#           pending_cart_charges_seen = 0
#           failed_cart_charges_seen = 0
#
#           subject.charges.each do |i|
#             pending_charges_seen_count += 1 if i.pending?
#             failed_charges_seen_count += 1 if i.failed?
...
#
#           subject.cart_items.each do |i|
#             cart_item_without_holdable_seen = true if i.holdable.blank?
#             cart_item_without_successful_charges_seen = true if !i.succesful_direct_charges?
#             fully_paid_cart_item_seen = true if i.successful_direct_charge_total >= i.price_cents
#           end
#
#           expect(pending_charges_seen_count).to be > 0
#           expect(failed_charges_seen_count).to be > 0
#           expect(pending_charges_seen_count + failed_charges_seen_count).to eql(subject.charges.size)
#
#           expect(cart_item_without_holdable_seen).to be_falsey
#           expect(cart_item_without_successful_charges_seen).to be_falsey
#           expect(fully_paid_cart_item_seen).to be_falsey
#         end
#       end
#
#       context "when the status is for_now" do
...
#         end
#
#         it "has its status set to 'awaiting_cheque'" do
#           expect(subject.status).to eql(Cart::AWAITING_CHEQUE)
#         end
#
#         it 'is valid' do
...
#
#       subject { ValidationPotato.new(charge_ary: :lots_of_failed_full_membership_charges + :lots_of_pending_full_membership_charges, cart_item_ary: :paid_cart_item_ary) }
#
#       context "when we're pre-validating the test-group" do
#         xit 'has a test-cart that is properly set up' do
#           expect(subject).to be
...
#           cart_item_without_holdable_seen = false
#           cart_item_without_successful_charges_seen = false
#           incompletely_paid_cart_item_seen = false
#
#           pending_cart_charges_seen = 0
#           failed_cart_charges_seen = 0
#
...
#             cart_item_without_holdable_seen = true if i.holdable.blank?
#             cart_item_without_successful_charges_seen = true if !i.succesful_direct_charges?
#             incompletely_paid_cart_item_seen = true if i.successful_direct_charge_total < i.price_cents
#           end
#
#           expect(pending_charges_seen_count).to be > 0
#           expect(failed_charges_seen_count).to be > 0
...
#           expect(cart_item_without_holdable_seen).to be_falsey
#           expect(cart_item_without_successful_charges_seen).to be_falsey
#           expect(incompletely_paid_cart_item_seen).to be_falsey
#         end
#       end
#
#       context "when the status is for_now" do
...
#     subject { ValidationPotato.new(charge_ary: :successful_small_charges) }
#
#
#     context "when the cart's charges, combined with the cart_items' independent charges, are LESS THAN the price of the cart's contents"
#
#       subject.cart_items { cart_item_ary: :partially_paid_cart_item_ary }
#
#       context "when we're pre-validating the test-group" do
#         xit 'has a test-cart that is properly set up' do
#
...
#           expect(subject.active?).to be_truthy
#
#           total_price_for_cart_items = 0
#           total_amount_owed_for_cart_items = 0
#           total_succesful_charges_for_cart = 0
#
#           pending_cart_charges_seen = 0
#           failed_cart_charges_seen = 0
#
