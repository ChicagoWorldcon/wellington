...
RSpec.describe Import::KansaMembers do
  let(:data) { "" }
  let(:read_stream) { StringIO.new(data) }
  let(:standard_headings) { Import::KansaMembersRow::HEADINGS.join(",") }
  let(:description) { "test stream" }
  subject(:command) { Import::KansaMembers.new(read_stream, description) }
  ...
  context "with import data" do
    let(:data) do
      CSV.generate do |csv|
        csv << Import::KansaMembersRow::HEADINGS
        csv << row_1
        csv << row_2
      end
    end
    let(:email_address) { "test@matthew.nz" }
    let(:good_row_processor) { instance_double(Import::KansaMembersRow, call: true) }
    let(:bad_row_processor) { instance_double(Import::KansaMembersRow, call: false, error_message: "gah") }

    let(:row_1) { ["1"] * Import::KansaMembersRow::HEADINGS.count }
    let(:row_2) { ["2"] * Import::KansaMembersRow::HEADINGS.count }

    it "calls ProcessRow" do
      expect(Import::KansaMembersRow)
