...
RSpec.describe Money::ChargeCustomer do
  ...
  describe "#comment" do
    ...
    it "displays purchase state as we go" do
      ...
      expect(Charge.last.comment).to eq(ChargeDescription.new(Charge.last).for_users)
  ...
  describe "mocking charge description" do
    ...
    context "when paying for reservations" do
      ...
      context "when payment fails" do
        ...
        it "creates a failed payment on card decline across all reservations" do
      ...
      context "when payment succeeds" do
        ...
        it "creates a new successful charge shared across reservations" do
    ...
    context "when paying off multiple reservations" do
      include_context "multiple reservations"

      let(:partial_pay) { Money.new(0) }
      let(:remainder) { full_amount_owed - partial_pay }
      let(:amount_owed) { remainder }

      subject(:command) {
        described_class.new(reservations, user, token, amount_owed, charge_amount: remainder)
      }

      it "transitions from instalment" do
        expect { command.call }
          .to change { reservations.map(&:state) }
          .from([Reservation::INSTALMENT] * reservations.count)
          .to([Reservation::PAID] * reservations.count)
      end

      it "creates a single charge" do
        expect { command.call }
          .to change { Charge.count }
          .by(1)
      end

      it "only pays the amount provided" do
        p "Paying #{remainder} out of #{ full_amount_owed }"
        p reservations
        p memberships

        command.call
        expect(user.charges.successful.sum(&:amount)).to eq remainder
      end

      it "only pays the price of the memberships" do
        command.call
        expect(user.charges.successful.sum(&:amount)).to eq memberships.sum(&:price)
      end
